...
public class ClassGrade910 extends JFrame {
    ...
    public ClassGrade910(String grade) {
        c = grade;
   ...
   table.getColumnModel().getColumn(7).setPreferredWidth(50);
    ...
    public void getData(){
     try {
         ...
         rs=stmt.executeQuery("SELECT n.terminal,(n.english_practical+n.compulsory_english) AS eng,(n.computer_practical+computer) AS comp,(n.science_practical+n.science) AS science,(n.health_pop_env_practical+n.health_pop_env_edu) AS health,n.compulsory_nepali AS nepali,n.compulsory_mathematics AS maths,n.social_studies AS social,n.optional_mathematics AS opt ,r.student_id,r.fname,r.mname,r.lname,r.class,r.roll_no  from " + c + " n JOIN record r ON n.sid = r.student_id ");
            while(rs.next()){
                   model.addRow(new Object[]{rs.getInt("student_id"), rs.getString("fname") + " " + rs.getString("mname") + " " + rs.getString("lname"), rs.getString("class"), rs.getString("roll_no"),rs.getString("terminal"),GettingGPA(getGPA(getGrade(rs.getDouble("eng")))+getGPA(getGrade(rs.getDouble("comp")))+getGPA(getGrade(rs.getDouble("health")))+getGPA(getGrade(rs.getDouble("nepali")))+getGPA(getGrade(rs.getDouble("maths")))+getGPA(getGrade(rs.getDouble("social")))+getGPA(getGrade(rs.getDouble("opt")))+getGPA(getGrade(rs.getDouble("science")))),percent,getRemarks(percent)});
            }
    ...
    public String GettingGPA(double zz){
      ...
      DecimalFormat dec = new DecimalFormat("0.00");
         percent= Double.parseDouble(dec.format((zz / 8.0)));
       if (percent >= 3.6) {
            finalgrade = "A+";
