...
public class ViewGrade15 extends JFrame {
    ...
    public ViewGrade15(String grade, int id) {
        c = grade;
        stid = id;
        add(panel);
...
//        new ViewGrade15("Five",130);
    ...
    public void getData(){
     try {
         ...
         pstmt=db.cn.prepareStatement("SELECT e.terminal,(e.english_I_practical+e.english_I) AS eng1,(e.english_II_practical+e.english_II) AS eng2, (e.computer_practical+e.computer) AS computer, (e.science_practical+e.science) AS science,(e.social_practical+e.social_studies) AS social,e.compulsory_nepali AS nepali,e.compulsory_mathematics AS maths,e.g_k AS gk,e.drawing AS draw,e.dictation AS dictation,e.moral AS moral,r.student_id,r.fname,r.mname,r.lname,r.class,r.roll_no FROM " + c + " e JOIN record r ON e.sid=r.student_id WHERE e.sid= ?"); 
         pstmt.setInt(1, stid);
            rs = pstmt.executeQuery();
            while(rs.next()){
                   model.addRow(new Object[]{rs.getInt("student_id"), rs.getString("fname") + " " + rs.getString("mname") + " " + rs.getString("lname"), rs.getString("class"), rs.getString("roll_no"),rs.getString("terminal"),GettingGPA(getGPA(getGrade(rs.getDouble("eng2")))+getGPA(getGrade(rs.getDouble("eng1")))+getGPA(getGrade(rs.getDouble("computer")))+getGPA(rs.getString("draw"))+getGPA(rs.getString("dictation"))+getGPA(getGrade25(rs.getDouble("gk")))+getGPA(getGrade25(rs.getDouble("moral")))+getGPA(getGrade(rs.getDouble("nepali")))+getGPA(getGrade(rs.getDouble("maths")))+getGPA(getGrade(rs.getDouble("social")))+getGPA(getGrade(rs.getDouble("science")))),percent,getRemarks(percent)});
            }
    ...
    public String GettingGPA(double zz){
      ...
      DecimalFormat dec = new DecimalFormat("0.00");
          percent= Double.parseDouble(dec.format((zz / 11.0)));
       if (percent >= 3.6) {
            finalgrade = "A+";
