...
public class ViewGrade8 extends JFrame {
    ...
    double percent1,percent,total,eng;
    int stid;

    public ViewGrade8(String grade, int id) {
        c = grade;
        stid = id;
        add(panel);
...
//        new ViewGrade8("Eight",133);
    ...
    public void getData(){
     try {
         ...
         pstmt=db.cn.prepareStatement("SELECT e.terminal,(e.computer_practical+e.computer) AS computer,(e.english_practical+e.compulsory_english) AS english, (e.science_practical+e.science) AS science,(e.social_practical+e.social_studies) AS social,e.compulsory_nepali AS nepali,e.compulsory_mathematics AS maths,e.vocation_and_occupation AS vocation,e.optional_mathematics AS opt,e.health as health,r.student_id,r.fname,r.mname,r.lname,r.class,r.roll_no FROM eight e JOIN record r ON e.sid=r.student_id WHERE e.sid= ?"); 
         pstmt.setInt(1, stid);
            rs = pstmt.executeQuery();
            while(rs.next()){
                   model.addRow(new Object[]{rs.getInt("student_id"), rs.getString("fname") + " " + rs.getString("mname") + " " + rs.getString("lname"), rs.getString("class"), rs.getString("roll_no"),rs.getString("terminal"),GettingGPA(getGPA(getGrade(rs.getDouble("english")))+getGPA(getGrade(rs.getDouble("computer")))+getGPA(getGrade(rs.getDouble("science")))+getGPA(getGrade(rs.getDouble("nepali")))+getGPA(getGrade(rs.getDouble("maths")))+getGPA(getGrade(rs.getDouble("social")))+getGPA(getGrade(rs.getDouble("opt")))+getGPA(getGrade50(rs.getDouble("vocation")))+getGPA(getGrade50(rs.getDouble("health")))),percent,getRemarks(percent)});
            }
    ...
    public String GettingGPA(double zz){
      ...
      DecimalFormat dec = new DecimalFormat("0.00");
        percent= Double.parseDouble(dec.format((zz / 9.0)));
       // percent=Double.parseDouble(dec.format(percent1));
       if (percent >= 3.6) {
            finalgrade = "A+";
